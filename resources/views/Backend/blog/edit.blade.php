...
@section('content')
    <section class="p-6">
        @include('../../message')
        <form action="{{ route('blog.update') }}" method="POST" class="bg-white p-6 rounded-lg shadow-lg"
            ...
            <div class="mb-6">
                ...
                <img height="50px" width="200px" src="{{ asset('storage/' . $blog->photo) }}" id="photo"
                    alt="select photo">
            </div>
                ...
                <select id="category" name="categorie_id"
                    ...
                    @if ($categories->isNotEmpty())
                        @foreach ($categories as $category)
                            @if ($category->name == $blog->categories->name)
